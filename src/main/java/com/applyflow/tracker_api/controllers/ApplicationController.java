...
public class ApplicationController {
    ...
    public ResponseEntity<ApiResponse<ApplicationResponseDto>> createApplication(
        ...
        ApplicationResponseDto response = applicationService.createAndCompileApplication(dto);
        return new ResponseEntity<>(
                ApiResponse.success("Application created and email compiled successfully", response),
                HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<ApiResponse<ApplicationResponseDto>> getApplicationById(@PathVariable Long id) {
        Long userId = securityContextService.getCurrentUserId();
        ApplicationResponseDto response = applicationService.getApplicationByIdAndUser(id, userId);
    ...
    public ResponseEntity<ApiResponse<ApplicationResponseDto>> patchApplicationStatusOrNotes(
        ...
        Long userId = securityContextService.getCurrentUserId();
        ApplicationResponseDto response = applicationService.updateApplicationStatusOrNotes(id, userId, status, notes);
        return ResponseEntity.ok(ApiResponse.success("Application state modified successfully", response));
    }
