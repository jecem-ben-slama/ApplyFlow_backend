...
public interface ApplicationRepository extends JpaRepository<Application, Long> {
        ...
        @Query("""
                        SELECT new com.applyflow.tracker_api.dtos.StatsStatusCountDto(a.status, COUNT(a))
                        FROM Application a
                        LEFT JOIN a.template t
                        LEFT JOIN a.cvVariant cv
                        WHERE a.user.id = :userId
                        ...
                        GROUP BY a.status
                        """)
        List<StatsStatusCountDto> countByUserIdAndStatusesInFilters(@Param("userId") Long userId,
                        @Param("statuses") List<String> statuses,
                        @Param("from") LocalDateTime from,
                        @Param("to") LocalDateTime to,
                        @Param("jobTitle") String jobTitle,
